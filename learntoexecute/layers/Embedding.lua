...
function Embedding:__init(inputSize, outputSize)
  ...
  self.weight = torch.Tensor(inputSize, outputSize)--就是分配一个空间，生成一个inputsize×outputsize的tensor
...
function Embedding:accGradParameters(input, gradOutput, scale)--相对于其自身的参数计算模块的梯度。 许多模块不执行此步骤，因为它们没有任何参数。 参数的状态变量名称取决于模块。 模块期望在某些变量中累积关于参数的梯度
  scale = scale or 1
  if scale == 0 then

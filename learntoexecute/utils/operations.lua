...
function small_loop_opr(hardness)
  ...
  local code = {string.format("%s=%s", var, a.expr),
                string.format("for x in range(%d):%s%s=%s", loop, var,
                                                            op, b.expr, var)}
  local expr = var
  return code, expr, eval--ode_tmp, var_tmp, output_tmp对应上一层
end

function ifstat_opr(hardness)
  local r_small = hardness()
  local a, b, c, d = get_operands(hardness, 4)--返回4,3,1,10
  if random(2) == 1 then
    name = ">"
  ...
  return {}, expr, output --code_tmp, var_tmp, output_tmp对应上一层的
end
